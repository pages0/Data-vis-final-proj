...
function displayData(FileName) {
  ...
  // Create new variables for the plot
  ...
  .await( function(error, data_stock, data_google_trends ) {
    ...
    data_google_trends=data_google_trends.filter(function(d) {
        return d.Date < maxExtent && d.Date > minExtent;
      });
    data_stock=data_stock.filter(function(d) {
        return d.Date < maxExtent && d.Date > minExtent;
    });
    ...
    svg.selectAll('.stock-tip')
         ...
         .attr('cy', function(d){
            ...
            d3.selectAll('div.stock-tip-text').remove();
	          d3.select(this)
		          .transition()
              .style('border','2px solid steelblue')
		            .attr('r',5);
	          d3.select('#stock-date')
		          .text("Date: " + (d.Date.getMonth()+1)+'/'+d.Date.getUTCDate()+'/'+d.Date.getFullYear());
